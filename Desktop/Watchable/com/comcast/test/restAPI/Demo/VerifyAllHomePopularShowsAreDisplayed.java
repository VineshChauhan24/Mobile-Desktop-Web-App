...
public class VerifyAllHomePopularShowsAreDisplayed extends BaseTest {
	...
	public void testVerifyAllHomePopularShowsAreDisplayed() throws Exception {
		...
		try {
			...
			// String
			// isNextEnable1=driver.findElement(By.xpath(".//*[@id='popular_shows']/div/section/a[2]/span")).getAttribute("class");
			...
			if (popularShowList != null && popularShowList.size() < 15) {
				for (VideoDetails showList : popularShowList) {
					...
					do {
						...
						isPresent = driver
								.findElement(
										By.xpath(".//*[@id='popular_shows']/div/section"))
								.findElements(By.linkText(showList.getTitle()))
								.size() > 0;
						if (isPresent == true) {
							isVideoPresent = true;
							assertTrue(driver
							...
							// This method asserts Home and Subscriptions
							// inactive link when user clicks on
							// Bundle/Channel/Show.
							...
							// This method asserts Search Text Box and its
							// value.
				...
				if (!isVideoPresent) {
					assertTrue(driver
							...
							.matches(
									"^[\\s\\S]*"
											+ popularShowList.get(0).getTitle()
											+ "[\\s\\S]*$"));
				}
			} else {
