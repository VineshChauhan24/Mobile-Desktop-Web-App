...
/**
 * Class Name: VerifyFollowButtonLabelAfterSuccessfulFollow Description: This
 * test script verifies the change in follow button label after successfully
 * following a show from show page, Change in label on mouse over after follow.
 *  Author: Manoj
 * **/

public class VerifyFollowButtonLabelAfterSuccessfulFollow extends BaseTest {

	@Test
	public void testVerifyFollowButtonLabelAfterSuccessfulFollow()
			throws Exception {

		try {

			// Test script
			log.info("Script: VerifyFollowButtonLabelAfterSuccessfulFollow");
			log.info("****************************************************");
			...
			if (loginError.isDisplayed() == false) {
				...
				if (channelCount > 0) {
					...
					if (!follow.contains(UILablesRepo.FOLLOW)) {
						...
						String followBtnLabelBeforeFollow = driver.findElement(
								By.xpath(XpathObjectRepo.followButton_XPATH))
								.getText();

						log.info("followBtnLabelBeforeFollow:"
								+ followBtnLabelBeforeFollow);

						// Click on follow button
						FollowFun.clickOnFollowButton();

						String followBtnLabelAFterFollow = driver.findElement(
								By.xpath(XpathObjectRepo.followButton_XPATH))
								.getText();

						// Verify text on follow button after successful follow
						assertTrue(
								"After following the label of the Follow button is NOT changed to "
										+ followBtnLabelAFterFollow,
								followBtnLabelAFterFollow
										.toUpperCase()
										.trim()
										.matches(
												UILablesRepo.TXT_FOLLOWING
														.toUpperCase().trim()));
						log.info("After following the label of the Follow button is changed to "
								+ followBtnLabelAFterFollow);

						// Mouse over followe button
						CommonFun
								.mouseOverElement(
										driver,
										driver.findElement(By
												.xpath(XpathObjectRepo.followButton_XPATH)));

						String followBtnLabelMouseOverAFterFollow = driver
								.findElement(
										By.xpath(XpathObjectRepo.followButton_XPATH))
								.getText();

						log.info("followBtnLabelMouseOverAFterFollow:"
								+ followBtnLabelMouseOverAFterFollow);

						// Verify text on follow button on mouse over
						assertTrue(
								"After following the label of the Follow button is NOT changed to "
										+ followBtnLabelMouseOverAFterFollow
										+ "On Mouse over",
