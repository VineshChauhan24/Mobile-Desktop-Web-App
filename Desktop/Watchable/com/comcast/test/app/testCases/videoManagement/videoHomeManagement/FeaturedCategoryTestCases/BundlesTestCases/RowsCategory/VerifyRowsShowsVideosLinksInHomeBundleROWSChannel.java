...
/**
 * Class Name: VerifyRowsShowsVideosLinksInHomeBundleROWSChannel Description:
 * This test case is to verify and ensure if 'Rows','Shows' and 'Videos' links
 * are present for a Bundle channel ROWS Category displayed under 'Featured'
 * section on 'Home' screen by logging into Gazeebo application with registered
 * user.
 */

public class VerifyRowsShowsVideosLinksInHomeBundleROWSChannel extends BaseTest {
	...
	@Test
	public void testVerifyRowsShowsVideosLinksInBundleROWSChannel()
			throws Exception {
		...
		try {
			...
			Thread.sleep(sleepTime);
			assertTrue(driver.findElement(By.cssSelector("BODY")).getText()
					.matches("^[\\s\\S]*ROWS[\\s\\S]*$"));

			assertTrue(driver.findElement(By.cssSelector("BODY")).getText()
					.matches("^[\\s\\S]*SHOWS[\\s\\S]*$"));

			assertTrue(driver.findElement(By.cssSelector("BODY")).getText()
					.matches("^[\\s\\S]*VIDEOS[\\s\\S]*$"));
