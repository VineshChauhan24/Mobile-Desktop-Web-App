...
/**
 * Class Name: VerifySubscribedChannelsGenreChannels Description: This test case
 * verifies Genre Categories channels in Subscribed channels CATEGORIES page. by
	...
	public void testVerifySubscribedChannelsGenreChannels() throws Exception {
		...
		try {
			...
			if (loginError == 0) {
				...
				List<VideoDetails> genresCategoryChannelsList = genreDetails
						.get("GenreCategoriesChannelList");
				...
				if (genresCategoryList != null) {
					...
					for (int index = 0; index < loopIndexMax; index++) {
						String getCategoryTitle = driver
								...
								.getText();
						// This is to assert Genre category from API response
						// and UI displayed.
						assertEquals(genresCategoryList.get(index).getTitle(),
								getCategoryTitle);

						if (genresCategoryChannelsList != null) {
							int channelloopIndexMax = 0;
							if (genresCategoryChannelsList.size() < 2) {
								channelloopIndexMax = genresCategoryChannelsList
										.size();
							} else {
							...
							for (int channelsIndex = 0; channelsIndex < channelloopIndexMax; channelsIndex++) {
								VideoDetails subscribedGenreChannels = genresCategoryChannelsList
										.get(channelsIndex);
								assertTrue(driver
					...
					assertTrue(driver
							...
							.getText()
							.matches(
									"^[\\s\\S]*You have not subscribed any channel yet.[\\s\\S]*$"));
				}
