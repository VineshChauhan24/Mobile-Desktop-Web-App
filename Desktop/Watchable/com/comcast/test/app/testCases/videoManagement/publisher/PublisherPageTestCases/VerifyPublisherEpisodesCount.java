...
/**
 * Class Name: VerifyPublisherEpisodesCount Description: This test case
 * validates whether Episodes count is displayed in publisher details page by
 * logging registered user into Watchable application.
 * **/

public class VerifyPublisherEpisodesCount extends BaseTest {
	...
	@Test
	public void testVerifyPublisherEpisodesCount() throws Exception {
		...
		try {
			...
			if (channelList != null) {
				...
				for (int index = 0; index < loopIndex; index++) {
					...
					String episodesCount = driver
							.findElement(
									By.xpath(XpathObjectRepo.PUBLISHERPAGE_VIDEOSCOUNT_XPATH))
							.getText();
					assertEquals("Videos "
							+ publishersDetails.get(0).getNumOfVideos() + "",
							episodesCount);
