...
/**
 * Class Name: VerifyReportFunctionalityWithLogin Description: This test case is
 * to verify Report functionality in video player by logging into Watchable
 * application.
 * **/
				...
				if (videoList != null) {
					...
					for (int i = 0; i < videoList.size(); i++) {
						...
						try {
							...
							System.out
									.println("-------------REPORT ELEMENT IN VIDEO PAGE IS NOT FOUND--------------");
						}
						...
						if (isReportPresent) {
							...
							assertTrue(driver
									...
									.getText()
									.equalsIgnoreCase(
											UILablesRepo.VIDEO_REPORT_HEADER));
							boolean isReportedBefore = false;
							try {
								WebElement element = driver
										.findElement(By
												.xpath("//div[@id='reportVideo']/descendant::input[@checked='checked']"));
							...
							if (isReportedBefore) {
								System.out
										.println("-------------VIDEO IS ALREADY REPORTED BY THE SAME USER--------------");
								driver.navigate().back();
								// i++;
								Thread.sleep(sleepTime);
							} else {
								driver.findElement(
										By.xpath(XpathObjectRepo.VIDEO_REPORTVIDEO_INCORRECTRATING_RADIO_XPATH))
										.click();
								driver.findElement(
										By.xpath(XpathObjectRepo.VIDEO_REPORTVIDEO_BUTTON_XPATH))
										.click();
								Thread.sleep(sleepTime);
								assertTrue(driver
										.findElement(
												By.xpath(XpathObjectRepo.VIDEO_VIDEOREPORTED_SPAN_XPATH))
										.getText()
										.matches(
												UILablesRepo.VIDEO_VIDEOREPORTED_SPAN_TEXT));
								Thread.sleep(sleepTime);
