package comcast.test.app.testCases.videoManagement.videoHomeManagement.FeaturedCategoryTestCases.BundlesTestCases.ChannelsCategory.ShowsCategory;
...
/**
 * Class Name: VerifyVideoCountForHomeBundleCHANNELS_SHOWS Description: This
 * test case is to verify the count of videos available for a Show displayed
 * under 'Home' bundle > CHANNELS > SHOWS for 'Featured' section on 'Store'
 * screen by logging into Comcast application.
 */

public class VerifyVideoCountForHomeBundleCHANNELS_SHOWS extends BaseTest {
	...
	@Test
	public void testerifyVideoCountForHomeBundleCHANNELS_SHOWS()
			throws Exception {
		...
		try {
			...
			System.out.println(EpisodeListUnderBundleSHOWS.get(
					XidioConstant.selectVideo).getNoOfHits());
			assertTrue(driver
					.findElement(By.cssSelector("BODY"))
					.getText()
					.matches(
							"^[\\s\\S]*Videos "
									+ EpisodeListUnderBundleSHOWS.get(
