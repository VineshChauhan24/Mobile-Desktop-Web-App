...
/**
 * Class Name: VerifyDuplicateSubscribedShowsWhenDoubleClickOnShows Description:
 * This test case is to verify if the duplicate shows are displayed after double
 * clicking on 'CATEGORIES' link for a subscribed channel displayed on My
 * Channels screen by logging into Gazeebo application.
 */

public class VerifyDuplicateSubscribedShowsWhenDoubleClickOnShows extends
		BaseTest {
	...
	@Test
	public void testVerifyDuplicateSubscribedShowsWhenDoubleClickOnShows()
			throws Exception {
		...
		try {
			...
			boolean isDuplicate = driver
					.findElements(
							By.xpath(".//*[@id='content-wrap']/article/section/article/div/div[3]/section/ul"))
					.size() > 0;
			if (isDuplicate == true) {
				String getShowsTitle = driver
						.findElement(
								By.xpath(".//*[@id='content-wrap']/article/section/article/div/div[3]/section/ul/li[1]/article/h1/a"))
						.getText();
				assertFalse(driver.findElement(By.cssSelector("BODY"))
						.getText()
						.matches("^[\\s\\S]*" + getShowsTitle + "[\\s\\S]*$"));
			}
			Thread.sleep(sleepTime);
			assertTrue(driver
					.findElement(By.cssSelector("BODY"))
					...
					.matches(
							"^[\\s\\S]*"
									+ subscribedChannelsShows.get(
											XidioConstant.selectSubscribedShow)
											.getTitle() + "[\\s\\S]*$"));
