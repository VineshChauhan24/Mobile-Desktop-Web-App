...
/**
 * Class Name: VerifyForwardFunctionality Description: This test case is to
 * verify Forward functionality in video player by logging into Watchable
 * application.
 * **/

public class VerifyForwardFunctionality extends BaseTest {
	...
	@Test
	public void testVerifyForwardFunctionality() throws Exception {
		...
		double endPosition, lastPosition, Postion, nextPosition;
				...
				if (videoList != null) {
					...
					if (nowPlaying) {
						...
						for (int i = 0; i < 2; i++) {
							...
							int currPostion = Integer.parseInt(currentPosition) + 30;

							if (currPostion < lastPosition) {
								homePageCommonFun.mouseOverOnVideoPlayer();
								driver.findElement(
										By.xpath(XpathObjectRepo.VIDEOFORWARDBUTTON_XPATH))
										.click();
								Thread.sleep(XidioConstant.tenSec);
								String forwardedPosition = flashApp
										.callFlashObject("getCurrentPosition");
								Double tempForwardedPosition = Double
										.parseDouble(forwardedPosition);
								forwardedPosition = Integer.valueOf(
										tempForwardedPosition.intValue())
										.toString();
								int forwarPositon = Integer
										.parseInt(forwardedPosition);

								assertEquals(forwarPositon, currPostion);

								currentPosition = TestDataGenerator
										.getFormattedTime(currentPosition);
								forwardedPosition = TestDataGenerator
										.getFormattedTime(forwardedPosition);

								String pausedPositionFromUI = driver
										.findElement(
												By.xpath(XpathObjectRepo.VIDEOCURRENTPOSITIONLABEL_XPATH))
										.getText();
								pausedPositionFromUI = pausedPositionFromUI
										.trim();

								assertEquals(forwardedPosition,
