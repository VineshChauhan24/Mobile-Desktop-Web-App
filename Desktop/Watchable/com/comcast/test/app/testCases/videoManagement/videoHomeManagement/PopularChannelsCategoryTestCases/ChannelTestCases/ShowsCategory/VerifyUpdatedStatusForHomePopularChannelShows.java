...
/**
 * Class Name: VerifyUpdatedStatusForHomePopularChannelShows Description: This
 * test case validates whether 'Updated' status is displayed in show page for
 * the show present in a channel under Popular Channels section in Home page by
...
public class VerifyUpdatedStatusForHomePopularChannelShows extends BaseTest {
	...
	@Test
	public void testVerifyUpdatedStatusForHomePopularChannelShows()
			throws Exception {
			...
			if (loginError == 0) {
				...
				if (channelList != null) {
					...
					for (int index = 0; index < loopMaxIndex; index++) {
						...
						driver.findElement(
								By.linkText(popularShowList.getTitle()))
								.click();

						Thread.sleep(sleepTime);

						// Manoj: Objects modified and added to XpathObjectRepo
						// file
						...
						assertTrue(driver
								...
								.getText().matches(popularShowList.getTitle()));
						...
						String publishedDate = driver
								...
								.getText();
						assertEquals(popularShowList.getLastPublished(),
								publishedDate);
