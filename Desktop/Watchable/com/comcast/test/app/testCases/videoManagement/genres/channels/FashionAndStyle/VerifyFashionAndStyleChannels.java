package comcast.test.app.testCases.videoManagement.genres.channels.FashionAndStyle;
...
/**
 * Class Name: VerifyFashionAndStyleChannels Description: This test case
 * validates whether channels are displayed in Fashion And Style Category
 * CHANNELS PAGE in Home page by with/without logging into Watchable
 * application.
 * **/

public class VerifyFashionAndStyleChannels extends BaseTest {
	...
	@Test
	public void testVerifyFashionAndStyleChannels() throws Exception {
		...
		try {
			...
			if (loginError == 0) {
								...
								for (int index = 0; index < loopMaxIndex; index++) {
									...
									for (; i <= listIndex; i++) {
										...
										log.info(driver
												...
												.getText());
										log.info(channelsList.getTitle());
				 ...
				 * ("BODY")).getText(
				 * ).matches("^[\\s\\S]*"+orProUtil.getProperty
				 * ("ANIMATION")+"[\\s\\S]*$"));
				 */
