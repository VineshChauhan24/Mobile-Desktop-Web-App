...
/**
 * Class Name: VerifyVideoCountForHomePopularChannelShows Description: This test
 * case validates whether 'Videos' count is displayed in show page for the show
 * present in a channel under Popular Channels section in Home page by logging
...
public class VerifyVideoCountForHomePopularChannelShows extends BaseTest {
	...
	@Test
	public void testVerifyVideoCountForHomePopularChannelShows()
			throws Exception {
				...
				if (channelList != null) {
					...
					for (int index = 0; index < loopMaxIndex; index++) {
						...
						int ele = driver.findElements(
								By.linkText(popularShowList.getTitle())).size();
						if (ele == 1) {
							driver.findElement(
							...
							// Manoj: Object changed and added to
							...
							assertEquals(
									"Videos "
											+ popularShowList.getNumOfVideos()
											+ "", Videocount);
