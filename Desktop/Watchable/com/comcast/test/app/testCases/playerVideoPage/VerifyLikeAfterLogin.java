...
/**
 * Class Name: VerifyLikeAfterLogin Description: This test script verify the
 * like functionality after login. Author: Manoj
 * **/

public class VerifyLikeAfterLogin extends BaseTest {
	...
	@Test
	public void testVerifyLikeAfterLogin() throws Exception {

		try {

			// Test script
			log.info("Script: VerifyLikeAfterLogin");
			log.info("******************************");
			...
			if (loginError.isDisplayed() == false) {
				...
				if (videoCount > 0) {
					...
					// verify whether the video is already liked

					if (driver
							.findElement(
									By.xpath(XpathObjectRepo.playerLikeButton_XPATH))
							.getAttribute("class").contains("liked")) {

						log.info("This video is already liked by you");

					} else {

						// Click on like button
						PlayerPageFun.clickOnLikeButton();
						log.info("Successfully liked the selected video");
