...
/**
 * Class Name: VerifyNavigatingToShowPageOnClickingShowTitleFromUpNextSection
 * Description: This test case verifies navigating to show page on clicking the
 * show title from Up Next section in Video page. Author: Manoj
 * **/

public class VerifyNavigatingToShowPageOnClickingShowTitleFromUpNextSection
		extends BaseTest {
	...
	@Test
	public void testVerifyNavigatingToShowPageOnClickingShowTitleFromUpNextSection()
			throws Exception {

		try {

			// Test script
			log.info("Script: VerifyNavigatingToShowPageOnClickingShowTitleFromUpNextSection");
			log.info("***********************************************************************");
			...
			if (videoCount > 0) {
				...
				// Verify show title in Up Next section

				assertTrue(
						"Show title is not displaying in Up Next section",
						CommonFun.isElementPresent(
								driver,
								By.xpath(XpathObjectRepo.videoPageUpNexSectionShowTitle_XPATH)));

				log.info("The show title displayed in Up Next section is '"
						+ driver.findElement(
								By.xpath(XpathObjectRepo.videoPageUpNexSectionShowTitle_XPATH))
								.getText() + "'");
				// Click on show title from up next section
				PlayerPageFun.clickOnShowTitle();

				// Verify successfully navigate to show details page
				AssertionRepoFunctions.assertChannelPageTitle();

			} else {
				log.error("What we're watching video section does not contain videos");
				log.info("");
			}
