...
/**
 * Class Name:
 * VerifyNavigatingToVideoPageClickingOnPlayListVideoDetailFromPlayList
 * Description: This test script verifies navigating video page after clicking
 * on play list video detail from play list. list. Author: Manoj
 * **/

public class VerifyNavigatingToVideoPageClickingOnPlayListVideoDetailFromPlayList
		extends BaseTest {
	AssertionRepoFunctions assertionFunction = new AssertionRepoFunctions();

	@Test
	public void testVerifyNavigatingToVideoPageClickingOnPlayListVideoDetailFromPlayList()
			throws Exception {

		try {

			log.info("Script: VerifyNavigatingToVideoPageClickingOnPlayListVideoDetailFromPlayList");
			log.info("*****************************************************************************");
			...
			if (playListCount > 0) {
				...
				// Verify Play List video detail
				assertTrue(
						"Play list video detail not present in play list",
						CommonFun.isElementPresent(
								driver,
								By.xpath(XpathObjectRepo.playlistVideoDetail_XPATH)));
				log.info("The video detail '"
						+ driver.findElement(
								By.xpath(XpathObjectRepo.playlistVideoDetail_XPATH))
								.getText() + "' is present in play list");

				// Click on play list video detail
				PlayListsFun.clickOnPlayListVideoDetailFromPlayList();
