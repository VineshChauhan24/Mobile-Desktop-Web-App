...
/**
 * Class Name: VerifyPlayListSharingWithoutLogin Description: This test script
 * verifies the login pop up displayed if user try to share play list through
 * email when user is not logged into application. 
 * Author: Manoj
 * **/

public class VerifyPlayListSharingWithoutLogin extends BaseTest {
	AssertionRepoFunctions assertionFunction = new AssertionRepoFunctions();

	@Test
	public void testVerifyPlayListSharingWithoutLogin() throws Exception {

		try {

			log.info("Script: VerifyPlayListSharingWithoutLogin");
			log.info("*****************************************");
			...
			if (playListCount > 0) {
				...
				assertTrue(
						"Login form is not displayed on selecting email sharing of playlist if user is not login to application",
						driver.findElement(
								By.xpath(XpathObjectRepo.loginForm_XPATH))
								.isDisplayed());
				log.info("Login form is displayed on selecting email sharing of playlist if user is not login to application");
