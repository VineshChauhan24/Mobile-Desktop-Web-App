...
/**
 * Class Name: VerifyingClickOnChannlNavigatingToChannelPage Description: This
 * test case verifies clicking on each channel title from provider page
 * navigating to shows detail page. Author: Manoj
 * **/

public class VerifyingClickOnChannlNavigatingToChannelPage extends BaseTest {
	...
	@Test
	public void testVerifyingClickOnChannlNavigatingToChannelPage()
			throws Exception {

		try {

			// Test script
			log.info("Script: VerifyingClickOnChannlNavigatingToChannelPage");
			log.info("*****************************************************");
			...
			if (channelCount > 0) {
				...
				log.info(channelCount
						+ " Shows are present in featured Watchable SHOWS OF THE WEEK in home page");
				...
				// Verify provider name in channel detail page
				assertTrue(
				...
				if (providePageChannelCount > 0) {
					if (providePageChannelCount >= 8) {
						...
						for (int i = 1; i <= 8; i++) {
							...
							// Click on each channel title

							log.info("Clicking on the Shows title '"
									+ driver.findElement(
							...
							Thread.sleep(sleepTime);
							// Verify successfully navigate to Shows details
							// page
							AssertionRepoFunctions.assertChannelPageTitle();
					...
					log.error("Shows are not present in chaneel section of provider detail page");
				}
				log.info("");
