package comcast.test.app.testCases.videoManagement.genres.channels.Comedy;
...
/**
 * Class Name: VerifyComedyChannels Description: This test case validates
 * whether channels are displayed in Comedy Category CHANNELS PAGE in Home page
 * by with/without logging into Watchable application.
 * **/

public class VerifyFunnyChannels extends BaseTest {
	...
	@Test
	public void testVerifyComedyChannels() throws Exception {
					...
					String getCategoryTitle = driver
							.findElement(
									By.xpath(XpathObjectRepo.HOMEGENRECHANNELSTITLELIST_XPATH
											+ "[" + category + "]")).getText();
					if (getCategoryTitle.equalsIgnoreCase(UILablesRepo.FUNNY)) {
						categoryFound = true;
						if (genreChannelsList != null) {
							int loopMaxIndex = 0;
							if (genreChannelsList.size() < 2)
								...
								for (; i <= listIndex; i++) {
									...
									if (ele1 == 1) {
										...
										WebElement channel = driver
												.findElement(By
														.xpath(XpathObjectRepo.HOMEGENRESCHNNLHEADERLABEL_XPATH));
