...
/**
 * Class Name: VerifyControlsOnVideoPlayerDuringPause Description:This test
 * script verifies the control displaying on player in video page while player
 * is in pause mode. Author: Manoj
 * **/

public class VerifyControlsOnVideoPlayerDuringPause extends BaseTest {
	...
	@Test
	public void testVerifyControlsOnVideoPlayerDuringPause() throws Exception {

		try {

			// Test script
			log.info("Script: VerifyControlsOnVideoPlayerDuringPause");
			log.info("**********************************************");
			...
			// This method is to scroll UI to What we're watching
			// section.
			...
			if (videoCount > 0) {
				...
				// Verify Play button is displaying on player when play is
				// paused
				assertTrue(
						"Play button is not displaying on player when play is paused",
						CommonFun.isElementPresent(driver, By
								.xpath(XpathObjectRepo.playerPlayButton_XPATH)));
				log.info("Play button is displaying on player when play is paused");

				// Verify Other controls displaying on player
				AssertionRepoFunctions.assertVideoPlayerControls();
