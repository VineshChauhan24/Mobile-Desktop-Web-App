...
/**
 * Class Name: VerifyPlayListEmailSharringFormCloseButton Description: This test
 * script verifies the functionality of close button in play list sharing pop up
 * window. 
 * Author: Manoj
 * **/

public class VerifyPlayListEmailSharringFormCloseButton extends BaseTest {
	AssertionRepoFunctions assertionFunction = new AssertionRepoFunctions();

	@Test
	public void testVerifyPlayListEmailSharringFormCloseButton()
			throws Exception {

		try {

			log.info("Script: VerifyPlayListEmailSharringFormCloseButton");
			log.info("******************************************************");
			...
			if (loginError.isDisplayed() == false) {
				...
				if (playListCount > 0) {
					...
					PlayListsFun.clickOnPlayListFormCloseIcon();

					// Verify Email sharing form is closed successfully
					assertFalse(
							"Play List sharing Form is not closed",
							driver.findElement(
									By.xpath(XpathObjectRepo.playlistemailShareForm_XPATH))
									.isDisplayed());
					log.info("Play List sharing Form is closed");
