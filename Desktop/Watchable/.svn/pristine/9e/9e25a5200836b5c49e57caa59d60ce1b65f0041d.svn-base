package comcast.test.app.testCases.videoManagement.genres.channels.Music;
...
/**
 * Class Name: VerifyMusicChannels Description: This test case validates whether
 * channels are displayed in Music Category CHANNELS PAGE in Home page by
 * with/without logging into Watchable application.
 * **/

public class VerifyMusicChannels extends BaseTest {
	...
	@Test
	public void testVerifyMusicChannels() throws Exception {

		Map<String, List<VideoDetails>> videoDetails = RestAPIServices
				.GenreCategoriesDetail(UILablesRepo.MUSIC);
					...
					for (int category = 1; category <= noOfCategoriesDisplayed; category++) {
						if (noOfCategoriesDisplayed <= 8) {
							String getCategoryTitle = driver
									...
									.getText();
							if (getCategoryTitle
									.equalsIgnoreCase(UILablesRepo.MUSIC)) {
								categoryFound = true;
