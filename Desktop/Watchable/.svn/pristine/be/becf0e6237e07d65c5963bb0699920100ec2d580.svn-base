 ...
 * Verifies watch button are displayed in all four videos displayed under
 * featured Play list section. Author: Manoj
 * **/

public class VerifyWatchButtonDisplayedInFeaturedPlaylistsVideos extends
		BaseTest {
	...
	@Test
	public void testVerifyWatchButtonDisplayedInFeaturedPlaylistsVideos()
			throws Exception {

		try {

			// Test script
			log.info("Script: VerifyWatchButtonDisplayedInFeaturedPlaylistsVideos");
			log.info("***********************************************************");
			...
			if (videoCount > 0) {
				...
				int watchButton = driver.findElements(
						By.xpath(XpathObjectRepo.playlistsWatchButton_XPATH))
						.size();
				if (watchButton > 0) {
					assertEquals(
							"Watch buttons is not present in all 6 videos in playlist",
							6, watchButton);
					log.info("Watch buttons is present in all 6 videos in playlist");
				}
