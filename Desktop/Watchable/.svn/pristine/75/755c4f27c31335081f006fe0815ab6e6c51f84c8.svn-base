...
/**
 * Class Name: VerifyPlayFromResumePoint Description: This test script verify
 * playing from resume pont of partially played videos. Author: Manoj
 * **/

public class VerifyPlayFromResumePoint extends BaseTest {
	...
	@Test
	public void testVerifyPlayFromResumePoint() throws Exception {

		try {

			// Test script
			log.info("Script: VerifyPlayFromResumePoint");
			log.info("*********************************");
			...
			if (loginError.isDisplayed() == false) {
				...
				if (videoCount > 0) {
					...
					int partillyPlayed = driver.findElements(
					...
					if (partillyPlayed > 0) {
						...
						if (driver
							...
							// Verify PLAY FROM RESUME point button is displayed
							assertTrue(
							...
							// Click on Play from resume point button
							driver.findElement(
									By.id(XpathObjectRepo.playFromResumeButton_ID))
									.click();
							Thread.sleep(sleepTime);
							log.info("Successfully clicked on play from resume point button");
