...
/**
 * Class Name: VerifyNavigatingToChannelPageFromVideoPage Description:This test
 * script verifies display of currently playing video title in video page.
 * Author: Manoj
 * **/

public class VerifyNavigatingToChannelPageFromVideoPage extends BaseTest {
	...
	@Test
	public void testVerifyNavigatingToChannelPageFromVideoPage()
			throws Exception {

		try {

			// Test script
			log.info("Script: VerifyNavigatingToChannelPageFromVideoPage");
			log.info("**************************************************");
			...
			if (videoCount > 0) {
				...
				// Scroll to video title
				// HomePageCommonFunctions.scrollToVideoResultSection();

				PlayerPageFun.clickonVideoPlayer();

				// Verify currently playing video channel title in video page
				assertTrue(
						"Currently playing video channel title is not present in video page",
						CommonFun.isElementPresent(
								driver,
								By.xpath(XpathObjectRepo.playingVideoChannelTitle_XPATH)));
				log.info("Currently playing video channel title is present in video page");

				// Logging channel title
				log.info("The title of the channel is: "
						+ driver.findElement(
								By.xpath(XpathObjectRepo.playingVideoChannelTitle_XPATH))
								.getText());

				// Click on channel title
				PlayerPageFun.clickOnChannelTitleLink();
