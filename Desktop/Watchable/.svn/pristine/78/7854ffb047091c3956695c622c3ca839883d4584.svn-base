...
/**
 * Class Name: VerifyVideoEmailSharringFromEmailContent Description: This test
 * script verifies the from email text field im email sharing of video windows
 * is prepopulated with logged in user email address. 
 * Author: Manoj
 * **/

public class VerifyVideoEmailSharringFromEmailContent extends BaseTest {
	@Test
	public void testVerifyVideoEmailSharringFromEmailContent() throws Exception {

		try {

			// Test script
			log.info("Script: VerifyVideoEmailSharringFromEmailContent");
			log.info("************************************************");
			...
			if (loginError.isDisplayed() == false) {
				...
				if (videoCount > 0) {
					...
					log.info("From email address text box is pre-populated with logged in user's email id");
