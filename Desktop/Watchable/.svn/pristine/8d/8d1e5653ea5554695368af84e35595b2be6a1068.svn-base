...
/**
 * Class Name: VerifyNavigatingToVideoPageClickingOnPlayListImgFromPlayList
 * Description: This test script verifies navigating video page after clicking
 * on play list image from play list. list. Author: Manoj
 * **/

public class VerifyNavigatingToVideoPageClickingOnPlayListImgFromPlayList
		extends BaseTest {
	AssertionRepoFunctions assertionFunction = new AssertionRepoFunctions();

	@Test
	public void testVerifyNavigatingToVideoPageClickingOnPlayListImgFromPlayList()
			throws Exception {

		try {

			log.info("Script: VerifyNavigatingToVideoPageClickingOnPlayListImgFromPlayList");
			log.info("********************************************************************");
			...
			if (playListCount > 0) {
				...
				// Verify play list image in Play List
				assertTrue(
						"Play list image not present in play list",
						CommonFun.isElementPresent(driver, By
								.xpath(XpathObjectRepo.playlistFirstIcon_XPATH)));
				log.info("Play list image present in play list");

				// Click on play list image
				PlayListsFun.clickOnPlayListImgFromPlayList();
