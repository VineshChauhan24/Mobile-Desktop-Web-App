...
/**
 * Class Name: VerifyPlayListSharingSendWithInvalidEmails Description: This test
 * script verifies clicking on send button from play list sharing pop up window
 * by entering invalid from and to email address displaying error message for
 * both fields. 
 * Author: Manoj
 * **/

public class VerifyPlayListSharingSendWithInvalidEmails extends BaseTest {
	AssertionRepoFunctions assertionFunction = new AssertionRepoFunctions();

	@Test
	public void testVerifyPlayListSharingSendWithInvalidEmails()
			throws Exception {

		try {

			log.info("Script: VerifyPlayListSharingSendWithInvalidEmails");
			log.info("************************************************");
			...
			if (loginError.isDisplayed() == false) {
				...
				if (playListCount > 0) {
					...
					PlayListsFun.emterFromToEmailAddress(UILablesRepo.INVALID_FORMAT_EMAIL);
