...
/**
 * Class Name: VerifyCurrentDuration Description:This test script verifies
 * whether current duration time value is displaying on the player. Author:
 * Manoj
 * **/

public class VerifyCurrentDuration extends BaseTest {
	...
	@Test
	public void testVerifyCurrentDuration() throws Exception {

		try {

			// Test script
			log.info("Script: VerifyCurrentDuration");
			log.info("*******************************");
			...
			if (videoCount > 0) {
				...
				assertTrue(
						"Video current played time is not displayed in video player",
						CommonFun.isElementPresent(
								driver,
								By.xpath(XpathObjectRepo.playerDisplayButton_XPATH)));
				log.info("Video current played time is displayed in video player");

				// Mouse over video player
				PlayerPageFun.mouseOverVideoPlayer();
				log.info("Video current played time is: "
						+ driver.findElement(
								By.xpath(XpathObjectRepo.playerDisplayButton_XPATH))
								.getText());
