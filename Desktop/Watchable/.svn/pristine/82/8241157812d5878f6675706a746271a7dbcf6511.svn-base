 ...
 * Verifies play icons are displayed in all four videos displayed under
 * featured Play list section. Author: Manoj
 * **/

public class VerifyPlayIconDisplayedInFeaturedPlaylistsVideos extends
		BaseTest {
	...
	@Test
	public void testVerifyPlayIconDisplayedInFeaturedPlaylistsVideos()
			throws Exception {

		try {

			// Test script
			log.info("Script: VerifyPlayIconDisplayedInFeaturedPlaylistsVideos");
			log.info("***********************************************************");
			...
			if (videoCount > 0) {
				...
				int playIcon = driver.findElements(
						By.xpath(XpathObjectRepo.playlistsPlayIcon_XPATH))
						.size();
				if (playIcon > 0) {
					assertEquals(
							"Play Icons is not present in all 6 videos in playlist",
							6, playIcon);
					log.info("Play Icons is present in all 6 videos in playlist");
				}
